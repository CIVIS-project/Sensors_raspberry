...
int Log2CSVFile(const char *path,  double Value, char ApartmentID[50]) {
    FILE    *hFile;
    uint32_t FileSize = 0;

//This section added for CIVIS project, managing RedisDB
    ...
    if (redis==NULL)
	{
	printf("The Redis Server is not available");
	}
	...
	sprintf(message,"%s\t%d-%02d-%02dT%02d:%02d:%02d%s\t%.1f\t%s", ApartmentID, tm.tm_year+1900, tm.tm_mon+1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, timezone, Value, "1");
	printf(message);
        reply=redisCommand(redis, "RPUSH Gas_measure_tail %s", message); 
        if (reply==NULL) printf("Error while inserting the Value in Redis\n");
	}
    redisFree(redis);
...
//Log Reading with date info to CSV File
int Log2File(char *DataPath, uint16_t mode, uint16_t meterindex, uint16_t infoflag, double metervalue, ecMBUSData *rfData, uint32_t ident, char ApartmentID[50]) {  //metervalue was erroneously declared as float, changed to double
    ...
    switch(mode) {
        case LOGTOCSV : sprintf(param, "/var/www/ecpiww/data/ecpiwwM%d.csv", meterindex+1);
                        Log2CSVFile(param, metervalue, ApartmentID); //log kWh
                        return APIOK;
                        break;
            ...
            if(wMBus_GetMeterDataList() > 0) {
                ...
                for(iX=0; iX<Meters; iX++) {
                    if((0x01<<iX) & wMBus_GetMeterDataList()) {
                        ...
                        Log2File(CommandlineDatPath, LogMode, iX, InfoFlag, csvValue, &RFData, ecpiwwMeter[iX].ident, ApartmentID);
